...
This project uses the setup generated by `create-react-app` alongside a Node Server.
...
This request is made to `localhost:3000`, the Webpack dev server. Webpack will infer that this request is actually intended for our API server. We specify in `package.json` that we would like Webpack to proxy GraphQL server `localhost:4000`:
...
1. If the user's browser tried to request `localhost:4000` directly, we'd run into issues with CORS.
...
```
heroku apps:create lmia-visualizer
```
